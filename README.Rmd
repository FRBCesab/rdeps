...
`library(foo)`, `require(foo)`, `foo::bar()`, and `use("foo", "bar")` and adds 
these dependencies to the `DESCRIPTION` file in the sections 
_Depends_, _Imports_, and _Suggests_.
...
Since the version **0.4**, `rdeps` introduces the `.rdepsignore` file. This file
can be used to ignore files or folders when screening files to detect 
dependencies. This file can be handled with the function 
...
package to identify external packages used in a project. R package version 0.4,
